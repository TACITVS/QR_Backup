...
from workers.encryption_worker import EncryptionWorker, EncryptionWorkerSignals
from workers.decryption_worker import DecryptionWorker, DecryptionWorkerSignals
...
class QRBackupApp(QWidget):
    ...
    def _start_encryption_worker(self, data: bytes, password: str, params: QRCodeParameters) -> None:
        ...
        signals = EncryptionWorkerSignals()
        worker = EncryptionWorker(data, password, params, signals)

        signals.success.connect(self._on_encryption_success)
        signals.error.connect(self._on_encryption_error)
    ...
    def _encrypt_file_and_generate_qr(self) -> None:
        ...
        params = self._get_qr_parameters(len(data))
        if not params:
            return

        self._start_encryption_worker(data, self.encrypt_file_pwd.text(), params)
    ...
    def _encrypt_text_and_generate_qr(self) -> None:
        ...
        params = self._get_text_qr_parameters(len(text_bytes))
        if not params:
            return

        self._start_encryption_worker(text_bytes, self.encrypt_text_pwd.text(), params)
    ...
    def _start_decryption_worker(self, qr_path: str, password: str) -> None:
        ...
        signals = DecryptionWorkerSignals()
        worker = DecryptionWorker(qr_path, password, signals)

        signals.success.connect(self._on_decryption_success)
        signals.error.connect(self._on_decryption_error)
    ...
    def _read_qr_and_decrypt(self) -> None:
        ...
        self._start_decryption_worker(self.qr_path.text(), self.decrypt_pwd.text())
    ...
    def _handle_camera_qr_code(self, data: str) -> None:
        ...
        self._start_decryption_worker(data, password)
